 ...
 *
 * Two shapes have to be read, because v0.4.0 replaced the hand-written enum
 * with one generated by the `diagnostic_rules!` macro. From that tag on
 * from_code matches `$($code => ...)`, a repetition rather than any literal, so
 * the codes have to come from the macro invocation instead.
 */
function codesAcceptedAtTag(tag: string): string[] | undefined {
    ...
    const generated = codesFromMacroInvocation(source);
    if (generated) {
        return generated;
    }
...
/**
 * The codes listed in the `diagnostic_rules!` invocation, or undefined when the
 * tag predates the macro.
 *
 * Only the invocation is read, never the `macro_rules!` definition above it:
 * the definition spells the arms as `$code`, so it names no code at all. Both
 * start at column 0, which is what tells them apart.
 */
function codesFromMacroInvocation(source: string): string[] | undefined {
    const opening = /^diagnostic_rules!\s*\{/m.exec(source);
    if (!opening) {
        return undefined;
    }
    const start = opening.index;
    // The invocation ends at the first closing brace back at column 0.
    const end = source.indexOf('\n}', start);
    const body = source.slice(start, end === -1 ? undefined : end);
    return [...body.matchAll(/=>\s*"([a-z][a-z-]*)"/g)].map((entry) => entry[1]);
}
